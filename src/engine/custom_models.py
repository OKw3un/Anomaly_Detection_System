...
class GraphDominantDetector(BaseDetector):
    ...
    def fit(self, X, y=None):
        ...
        if hasattr(self.model, "emb") and self.model.emb is not None:
            ...
            # GÃ¶mme katmanÄ± doÄŸrudan eriÅŸilemiyorsa dÃ¼ÄŸÃ¼m Ã¶zniteliklerini sakla
    ...
    def decision_function(self, X):
        ...
        return scores
# =================================================================
# 3. Supervised (GÃƒÂ¶zetimli) Zaman Serisi Modelleri
# =================================================================


class SupervisedXGBoost(BaseDetector):
    """
    Zaman serisi pencerelerini flatten ederek XGBoost SÃ„Â±nÃ„Â±flandÃ„Â±rÃ„Â±cÃ„Â± eÃ„Å¸itir.
    """
    def __init__(self, seq_len: int = 21, contamination: float = 0.05, random_state: int = 42):
        super().__init__(contamination=contamination)
        self.seq_len = seq_len
        self.random_state = random_state
        self.model = None

    def _flatten_windows(self, X: np.ndarray):
        n_samples = X.shape[0]
        pad_size = self.seq_len - 1
        pad_X = np.vstack([np.repeat(X[:1], pad_size, axis=0), X])
        windows = []
        for i in range(n_samples):
            windows.append(pad_X[i : i + self.seq_len].flatten())
        return np.array(windows, dtype=np.float32)

    def fit(self, X, y=None):
        if y is None:
            raise ValueError("SupervisedXGBoost requires 'y' (labels).")
        try:
            from xgboost import XGBClassifier
        except ImportError:
            raise ImportError("XGBoost kurulu deÃ„Å¸il. LÃƒÂ¼tfen 'pip install xgboost' ÃƒÂ§alÃ„Â±Ã…Å¸tÃ„Â±rÃ„Â±n.")

        X_flat = self._flatten_windows(np.asarray(X))
        
        num_pos = max(1, np.sum(y == 1))
        scale_pos_weight = (len(y) - num_pos) / num_pos
        ...
        self.decision_scores_ = self.model.predict_proba(X_flat)[:, 1]
        self._process_decision_scores()
        return self
...
class SupervisedLightGBM(BaseDetector):
    """
    Zaman serisi pencerelerini flatten ederek LightGBM SÃ„Â±nÃ„Â±flandÃ„Â±rÃ„Â±cÃ„Â± eÃ„Å¸itir.
    """
    def __init__(self, seq_len: int = 21, contamination: float = 0.05, random_state: int = 42,
                 is_unbalance: bool = True, min_child_samples: int = 20):
        super().__init__(contamination=contamination)
        self.seq_len = seq_len
        self.random_state = random_state
    ...
    def fit(self, X, y=None):
        if y is None:
            raise ValueError("SupervisedLightGBM requires 'y' (labels).")
        try:
            from lightgbm import LGBMClassifier
        except ImportError:
            raise ImportError("LightGBM kurulu deÃ„Å¸il. LÃƒÂ¼tfen 'pip install lightgbm' ÃƒÂ§alÃ„Â±Ã…Å¸tÃ„Â±rÃ„Â±n.")
        ...
        self.model.fit(X_flat, y)
        scores = self.model.predict_proba(X_flat)[:, 1]
        
        # PyOD Threshold Bug Fix: AÄŸaÃ§ modelleri aynÄ± yapraktaki tÃ¼m satÄ±rlara 
        # tÄ±pasa tÄ±p aynÄ± olasÄ±lÄ±ÄŸÄ± (Ã¶rn: 0.89500) verir. PyOD percentile hesabÄ± yapÄ±p 
        # "skor > threshold" dediÄŸinde, hepsi threshold'a EÅÄ°T olduÄŸu iÃ§in > koÅŸulu False dÃ¶ner ve 
        # tespit edilen anomali sayÄ±sÄ± 0'a dÃ¼ÅŸer. EÅŸitliÄŸi bozmak iÃ§in mikroskobik bir gÃ¼rÃ¼ltÃ¼ ekliyoruz.
        noise = np.random.uniform(0, 1e-7, size=len(scores))
        self.decision_scores_ = scores + noise
